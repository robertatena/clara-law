...
function analyzeBySector(fullText: string, sector: SectorKey, role: string, companyName: string = ""): { summary: string; findings: Finding[]; email: { subject: string; body: string } } {
  const text = fullText || "";
  ...
  if (has(/alterar unilateralmente|altera[cç][aã]o unilateral|poder[aá]\s+alterar\s+unilateralmente/i)) {
    add({
      title: "Alteração unilateral do contrato",
      whyItMatters: "A outra parte não deveria poder mudar condições importantes sozinha.",
      whatToDo: "Peça que qualquer mudança dependa de aviso claro e concordância das partes.",
  ...
  if (has(/reajustados unilateralmente|cobrados retroativamente|cobran[cç]a retroativa|reajuste unilateral/i)) {
    add({
      title: "Cobrança retroativa ou reajuste unilateral",
      whyItMatters: "Isso pode gerar cobranças inesperadas e difíceis de contestar.",
  ...
  if (has(/ren[uú]ncia de direitos|renuncia a direitos|abre m[aã]o de direitos/i)) {
    add({
      title: "Renúncia de direitos",
      whyItMatters: "Uma cláusula não deveria obrigar você a abrir mão de proteções básicas.",
      whatToDo: "Peça a retirada dessa cláusula ou uma redação que preserve seus direitos legais.",
  ...
  if (has(/confiss[aã]o de d[ií]vida|liquida e exig[ií]vel|qualquer cobran[cç]a ser[aá] considerada/i)) {
    add({
      title: "Confissão automática de dívida",
      whyItMatters: "Isso pode dificultar sua defesa caso exista cobrança indevida ou valor discutível.",
      whatToDo: "Peça remoção dessa previsão ou limite claro para quando ela realmente se aplica.",
  ...
  if (has(/foro|elei[cç][aã]o de foro/i)) {
    add({
      title: "Foro escolhido no contrato",
  ...
  if (has(/n[aã]o se responsabiliza|isenta de responsabilidade|sem responsabilidade|n[aã]o responde por/i)) {
    add({
      title: "Isenção excessiva de responsabilidade",
      whyItMatters: "A outra parte não pode simplesmente se livrar de toda responsabilidade.",
      whatToDo: "Peça uma redação equilibrada, com responsabilidades proporcionais para cada lado.",
      legalHint: "Busque isenção de responsabilidade, não responde, não se responsabiliza.",
      severity: "high",
      points: 10,
    });
  }
