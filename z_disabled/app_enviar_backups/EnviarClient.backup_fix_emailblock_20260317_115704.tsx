...
export default function EnviarClient() {
  ...
  const canGoStep3 = (leadEmail || "").trim().length > 0 && (leadRole || "").trim().length > 0;
  ...
  const complaintText = useMemo(() => {
    ...
    // Se o usuário marcou pontos, usa esses
    if (!selectedFindings.length) return base;
    ...
    return [
      "Olá, " + (companyName || "[NOME DA EMPRESA]") + ".",
      "Tenho um problema relacionado a um contrato (" + sectorLabel + "). Minha posição: " + (leadRole || "[MINHA POSIÇÃO]") + ".",
      "Resumo do problema:",
      (summary || "").replace(/\n/g, " ").trim(),
      "Pontos principais que identifiquei:",
      (chosen || "- Solicito análise e orientação sobre os pontos destacados."),
      "Peço, por favor, uma avaliação e uma proposta de solução por escrito.",
      "Obrigada."
    ].join("\n\n");
  }, [companyName, sectorLabel, leadRole, summary, selectedFindings]);
  // UI helpers
    ...
    <div style={{ background: "#fff", minHeight: "100vh", color: COLORS.ink }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 24px" }}>
        ...
        <div style={{ marginTop: 18, display: "grid", gap: 18 }}>
          ...
          {step === 2 && (
            <Card
              title="2) Para a análise completa, preencha seus dados"
              subtitle="Obrigatório: e-mail e sua posição no contrato. (Isso melhora a recomendação e o texto pronto.)"
            >
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div>
                  <div style={{ fontSize: 12, color: COLORS.muted, fontWeight: 800 }}>Nome</div>
                  <input value={leadName} onChange={(e) => setLeadName(e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: 12, border: "1px solid " + COLORS.line }} />
                </div>

                <div>
                  <div style={{ fontSize: 12, color: COLORS.muted, fontWeight: 800 }}>Telefone</div>
                  <input
                    value={leadPhone}
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  <div style={{ fontSize: 12, color: COLORS.muted, fontWeight: 800 }}>Nome da empresa (para Reclame Aqui/Procon) — opcional</div>
                  <input
                    value={companyName}
          ...
          {step === 3 && (
            <Card
              title="3) Créditos e análise"
              subtitle="Você pode analisar com créditos grátis (quando houver) ou comprar créditos. Depois clique em “Analisar agora”."
            >
              <BillingPanel email={leadEmail} freeLeft={billingFreeLeft} credits={billingCredits} />

              <div style={{ marginTop: 12, padding: 12, border: "1px solid " + COLORS.line, borderRadius: 16, background: COLORS.soft2 }}>
                <div style={{ fontSize: 12, color: COLORS.muted, fontWeight: 800 }}>Comprar</div>
                <select
                  value={buyPack}
                  onChange={(e) => setBuyPack(e.target.value as any)}
                  style={{ width: "100%", marginTop: 6, padding: "10px 12px", borderRadius: 12, border: "1px solid " + COLORS.line }}
                >
                  <option value="1">1 análise (R$ 5)</option>
                  <option value="5">5 análises (R$ 19)</option>
                  <option value="10">10 análises (R$ 29)</option>
                </select>

                <div style={{ marginTop: 8, fontSize: 12, color: COLORS.muted, lineHeight: 1.5 }}>
                  (Integração Stripe/PIX: você já tem componentes — aqui fica o “slot” do checkout.)
                </div>
                <PrimaryBtn
                  onClick={() => alert("Checkout (Stripe) entra aqui. Por enquanto, apenas rode a análise abaixo.")}
                  style={{ marginTop: 10 }}
                >
                  Pagar e continuar
                </PrimaryBtn>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
                ...
                <PrimaryBtn disabled={!canAnalyze} onClick={runAnalysis}>
                  {isAnalyzing ? "Analisando..." : "Analisar agora"}
                </PrimaryBtn>
              </div>

              <p style={{ marginTop: 12, color: COLORS.muted, fontSize: 12, lineHeight: 1.5 }}>
                Transparência: a Clara não é advogada. É uma ferramenta de apoio para entender melhor e decidir com mais segurança.
              </p>
